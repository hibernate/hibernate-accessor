...
public class HibernateAccessorByteBuddyFactory implements org.hibernate.accessor.bytebuddy.HibernateAccessorByteBuddyFactory {
	...
	private final org.hibernate.accessor.HibernateAccessorFactory reflectionFallback = org.hibernate.accessor.HibernateAccessorFactory.reflection();
	...
	private HibernateAccessorMultiValueReader generateBulkBasedReader(Member[] members, MultiValueAccessorPointcuts pointcuts) {
		...
		bytecodeDumper.dump( "org/hibernate/accessor/bytebuddy/impl/HibernateAccessorMultiBulkReader", bytecode );
		try {
			final MethodHandles.Lookup hiddenLookup = ACCESSOR_MODULE_LOOKUP.defineHiddenClass(bytecode, true);
			final Class<?>[] paramTypes = new Class<?>[layout.accessors.length];
			Arrays.fill(paramTypes, HibernateAccessorByteBuddyBulkAccessor.class);
	...
	private HibernateAccessorMultiValueWriter generateBulkBasedWriter(Member[] members, MultiValueAccessorPointcuts pointcuts) {
		...
		bytecodeDumper.dump( "org/hibernate/accessor/bytebuddy/impl/HibernateAccessorMultiBulkWriter", bytecode );
		try {
			final MethodHandles.Lookup hiddenLookup = ACCESSOR_MODULE_LOOKUP.defineHiddenClass(bytecode, true);
			final Class<?>[] paramTypes = new Class<?>[layout.accessors.length];
			Arrays.fill(paramTypes, HibernateAccessorByteBuddyBulkAccessor.class);
